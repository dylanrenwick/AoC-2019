import IntcodeMachine from "../Intcode/IntcodeMachine";

// hardcoded input because input handling is too much work
...
// Part 1
console.log(cpu.run(program, [12, 2]));

// Bruteforce part 2
for (let i = 0; i < 100; i++) {
    for (let j = 0; j < 100; j++) {
        let result = cpu.run(program, [i, j]);
